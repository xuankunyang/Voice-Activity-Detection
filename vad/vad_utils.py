...
def parse_vad_label(line, frame_size: float = 0.032, frame_shift: float = 0.008):
    ...
    """
    frame2time = lambda n: n * frame_shift + frame_size / 2 # 注意这里的1/2指的是当窗口中是否有超过一半的时间属于正类
    frames = []
    frame_n = 0
...
def prediction_to_vad_label(prediction, frame_size: float = 0.032, frame_shift: float = 0.008, threshold: float = 0.5):
