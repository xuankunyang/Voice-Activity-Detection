...
import numpy as np
from extracting_short_time_features import short_time_zero_crossing_rate, enframe
import librosa
...
def plot_visualizations(frames, audio):
    ...
    """
    zcr = short_time_zero_crossing_rate(frames=frames)
    ...
    axes[0].set_title(f'Waveform and Zero Crossing Rate for Frame {frame_index}')
    axes[0].set_xlabel('Samples')
    axes[0].set_ylabel('Amplitude')
    axes[0].text(0.98, 0.95, f'Zero Crossing Rate for Frame {frame_index}: {zcr[frame_index]}', 
             ha='right', va='top', transform=axes[0].transAxes,
             color='red', 
             bbox=dict(facecolor='white', edgecolor='gray', boxstyle='round,pad=0.5', linewidth=1))
    
    axes[1].plot(np.arange(frames.shape[0]), zcr, color='r')
    axes[1].set_title('Zero Crossing Rate per Frame')
