...
def Comparision_on_frame_size_and_shift_Logistic_Regression(frame_size: float, frame_shift: float):
    sampling_rate = 16000
    ...
    # 保存到 JSON 文件
    # log_dir = 'task1/logs'
    # log_file = os.path.join(log_dir, 'logistic_regression_counting_on_frame_size_and_shift_experiment_log.json')
    # with open(log_file, 'a') as f:
    #     json.dump(experiment_data, f, indent=4, cls=NumpyEncoder)
    #     f.write('\n')  # 每次日志记录一行

    # file_path = f'task1/models/Logistic_Regression/Logistic_Regression_for_frame_length_{frame_length}_step_{frame_step}.pkl'
...
if __name__ =='__main__':
    ...
    # frame_size_list = [float(0.256), float(0.064)]
    # frame_shift_list = [float(0.128), float(0.016)]
    ...
    for frame_size, frame_shift in tqdm(zip(frame_size_list, frame_shift_list), desc="Processing different pairs", unit="pair"):
        accuracy, auc, eer, predictions, truth, y_test, y_prob = Comparision_on_frame_size_and_shift_Logistic_Regression(frame_size=frame_size, frame_shift=frame_shift)
        all_auc.append(auc)
    ...
    # plt.title('frame size : frame shift = 4 (Logistic Regression)')
    # plt.savefig('task1/figs/Logistic_Regression_ratio_4.png')
    # plt.close()
    ...
    # plt.xlabel('frame size(log2) / ms')
    # plt.legend(['auc', 'acc', 'eer'])
    # plt.title('frame size : frame shift = 2 (Logistic Regression)')
    # plt.savefig('task1/figs/Logistic_Regression_2.png')
